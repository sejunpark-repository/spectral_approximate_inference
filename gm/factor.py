...
class Factor():
    def __init__(self,name = None, variables = [], **kwargs):
        ...
        if 'values' in kwargs:
            ...
            # with np.errstate(divide='ignore', invalid='ignore'): #fixed
        ...
        if len(variables) != self.log_values.ndim:
            raise ValueError("Shape {shape} of {name} does not match number {variable_num}."
                             .format(shape = self.log_values.shape, name = name,
    ...
    def initialize_with_(cls, name, variables, numpy_func, cardinality, **kwargs):
        return cls(name = name,
                   variables = variables,
                   values = numpy_func(cardinality),
                   **kwargs)

    @classmethod
    def full_like_(cls, factor, value):
        return cls(name = copy(factor.name),
    ...
    def copy(self, rename = False):
        if rename:
            return Factor(name = default_factor_name(),
                          variables = copy(self.variables),
                          log_values = np.copy(self.log_values))
        else:
            return Factor(name = copy(self.name),
                          variables = copy(self.variables),
                          log_values = np.copy(self.log_values))
    ...
    def normalize(self, variables = None, inplace = True):
        ...
        # with np.errstate(invalid = 'ignore'): #fixed
        fac.log_values = fac.log_values - logsumexp(fac.log_values,
    ...
    def product(self, fac1, name = None, inplace = True):
        ...
        if isinstance(fac1, (int, float)):
            ...
            for axis in range(fac.log_values.ndim):
                ...
                fac1.variables[axis], fac1.variables[exchange_index] = fac1.variables[exchange_index], fac1.variables[axis]
    ...
    def div(self, fac1, name = None, inplace = True):
        ...
        if isinstance(fac1, (int, float)):
            ...
            zero_indices = np.logical_and(~np.isfinite(fac.log_values),
                                          ~np.isfinite(fac1.log_values))
    ...
    def __eq__(self, other):
        if not issubclass(type(self), type(other)):
            return False
        if self.name == other.name and tuple(sorted(self.variables)) == tuple(sorted(other.variables)):
            return True
        else:
            return False
...
def logsumexp(a, axis = None, keepdims = False):
    ...
    tmp = exp(a - a_max)
    #with np.errstate(divide = 'ignore'): #fixed
